...
public class removecourse extends JFrame implements ActionListener, FocusListener {
    Connection cn;
    Statement st;
    JComboBox jc;
    ResultSet rs;
    ...
    public removecourse() {
        ...
        del.setBounds(1150, 420, 150, 40);
        c.add(del);
        del.addActionListener(this);
        back.addActionListener(this);

        setTitle("DELETE THE COURSE DETAILS");
    ...
    public void focusGained(FocusEvent fe) {
        try {
            ...
            String qr = "select * from course where CourseId ='" + str + "'";
            rs = st.executeQuery(qr);
            if (rs.next()) {
                t1.setText("Course Name:" + rs.getString(2));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (str.equals("BACK")) {
            ...
            try {
                String sql2 = "delete from course where CourseId='" + (String) jc.getSelectedItem() + "'";
                int r = st.executeUpdate(sql2);
                if (r > 0) {
